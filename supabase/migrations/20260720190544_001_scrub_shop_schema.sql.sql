/*
# اسكربك (ScrubShop) — Full E-commerce Schema

Production-ready schema for a premium Arabic medical-scrubs e-commerce platform.
Covers catalog (products, categories, colors, sizes, fabrics), commerce
(cart, orders, order_items, reviews, favorites), support (AI + human chat),
and CMS (banners, social links, FAQs, settings, notifications).
RLS enabled on every table. Idempotent. Sample data included.
*/
...
-- Reviews: anyone can post
CREATE POLICY "anyone_insert_reviews" ON reviews FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "owner_update_reviews" ON reviews FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
...
('whatsapp', 'واتساب', 'https://wa.me/0000000000', 'message-circle', '#25D366', 4)
ON CONFLICT DO NOTHING;
